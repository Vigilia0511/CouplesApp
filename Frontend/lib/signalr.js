...
export async function initializeSignalR(userId, coupleId) {
...
export function onReceiveOffer(callback) {
    if (connection) {
        connection.on("receiveOffer", callback);
    }
}
export function onReceiveAnswer(callback) {
    if (connection) {
        connection.on("receiveAnswer", callback);
    }
}
export function onReceiveIceCandidate(callback) {
    if (connection) {
...
export async function sendOffer(receiverId, coupleId, offer) {
    if (connection?.state === signalR.HubConnectionState.Connected) {
        await connection.invoke("SendOffer", receiverId, coupleId, offer);
    }
}
export async function sendAnswer(receiverId, coupleId, answer) {
    if (connection?.state === signalR.HubConnectionState.Connected) {
        await connection.invoke("SendAnswer", receiverId, coupleId, answer);
    }
}
export async function sendIceCandidate(receiverId, coupleId, candidate) {
    if (connection?.state === signalR.HubConnectionState.Connected) {
        await connection.invoke("SendIceCandidate", receiverId, coupleId, candidate);
    }
}
export async function endCall(coupleId) {
    if (connection?.state === signalR.HubConnectionState.Connected) {
        await connection.invoke("EndCall", coupleId);
    }
}
export async function rejectCall(coupleId) {
    if (connection?.state === signalR.HubConnectionState.Connected) {
