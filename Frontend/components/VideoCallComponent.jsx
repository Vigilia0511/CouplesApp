...
import { initializeSignalR, disconnectSignalR, sendOffer, sendAnswer, sendIceCandidate, endCall, onReceiveOffer, onReceiveAnswer, onReceiveIceCandidate, onCallEnded, onCallRejected, } from "@/lib/signalr";
...
export default function VideoCallComponent({ currentUserId, partnerId, coupleId, partnerName, }) {
    ...
    const waitForCallAcceptance = (callId) => {
    ...
    const handleReceiveOffer = async (offerJson) => {
        try {
    ...
    const handleReceiveAnswer = async (answerJson) => {
        try {
    ...
    const handleReceiveIceCandidate = async (candidateJson) => {
        try {
    ...
    return (<div className={styles.container}>
      <div className={styles.wrapper}>
        ...
        <div className={styles.controls}>
          {!isCallActive ? (<button className={styles.btnCall} onClick={initiateCall}>
              ?? Call Partner
            </button>) : (<>
              <button className={`${styles.btn} ${isMicEnabled ? styles.btnActive : styles.btnOff}`} onClick={toggleMicrophone} title="Toggle Microphone">
                ??
              </button>
              <button className={`${styles.btn} ${isVideoEnabled ? styles.btnActive : styles.btnOff}`} onClick={toggleCamera} title="Toggle Camera">
                ??
              </button>
              <button className={`${styles.btn} ${styles.btnEnd}`} onClick={handleEndCall} title="End Call">
                ?
              </button>
            </>)}
        </div>
      ...
      {showIncomingModal && incomingCall && (<IncomingCallModal callId={incomingCall.id} initiatorName={incomingCall.initiatorName} onAccept={async () => {
